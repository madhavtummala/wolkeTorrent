    ...
    while response is None:
        ...
        try:
            ...
            if error.resp.status in [404]:
                ...
                logger.warning('Upload [%s] fail http [%s] retry [%s] next_chunk: %s' %
            ...
            idle_count += 1
            logger.warning('Upload [%s] fail [socket.error] retry [%s] next_chunk: %s' % (input_file, idle_count, error))
            time.sleep(idle_count * idle_count * 3)
            continue
        except:
        ...
        if status:
            if last_progress >= status.resumable_progress:
                logger.warning('Upload [%s] retry [%s] next_chunk: No progress from last chunk' % (input_file, idle_count))
            else:
                idle_count = 0
            ...
            idle_count += 1
            logger.warning('Upload [%s] retry [%s] next_chunk: Not receive status from status, response = request.next_chunk()' % (input_file, idle_count))
...
input_file = '/downloads.zip'
import os
for file in os.listdir("/"):
    if file.endswith(".zip"):
        input_file = '/'+file

print("found input file {}".format(input_file))
...
time.sleep(1)
upload_file(service=service, input_file=input_file, folder_name="WolkeTorrent", show_progress=True)
